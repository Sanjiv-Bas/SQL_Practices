...
select sum(amount) as TotalAmount,cust_name
from sales
group by cust_name;

-- Total quantity sold per product
select prod_name, sum(prod_unit) as TotalQuantity
from sales
group by prod_name;

-- Sales amount per salesman
select salesman_name, sum(amount)as TotalAmount
from sales
group by salesman_name;

-- Number of units per customer
select cust_name, sum(prod_unit) as Totalunits
from sales
group by cust_name;

-- Average sales amount per customer
select cust_name, avg(amount) as Avgamount
from sales
group by cust_name;

-- 6. Highest sales per product
select prod_name, max(amount) as MaximumAmount
from sales
group by prod_name;

-- Lowest sales per salesman
select salesman_name, min(amount) as Minimumamount
from sales
group by salesman_name;

-- Number of different products sold by each salesman
select salesman_name,count(distinct prod_name) as Products
from sales
group by salesman_name;

-- Total revenue generated per product by each salesman
select salesman_name, sum(amount) as TotalAmount
from sales
group by salesman_name;

-- Customers who bought more than 2 units in total
select cust_name,sum(prod_unit) 
from sales
group by cust_name
having sum(prod_unit) > 2;
